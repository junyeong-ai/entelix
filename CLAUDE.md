...
9. **No filesystem, no shell** — first-party crates do NOT import `std::fs`, `std::path::Path`/`PathBuf` (except as opaque IDs), `std::process`, `std::os::unix::process`, `tokio::fs`, `tokio::process`, `landlock`, `seatbelt`, `tree-sitter*`, `nix`. Coding-shape tools (shell, code-exec, file-IO) live in `entelix-tools` behind the `sandboxed` feature flag and route every syscall through the operator-supplied `entelix_core::sandbox::Sandbox` trait — the import ban holds even with the feature enabled. **Documented exception**: credential-storage companion crates (today: `entelix-auth-claude-code`) may import `std::fs` *bounded to the OAuth credential file the upstream Claude Code CLI shares* — the exception is allowlisted per-crate in `cargo xtask no-fs`'s `CREDENTIAL_STORAGE_EXEMPTIONS`, and any new crate proposing fs access must amend this invariant and the gate's allowlist together. Enforced by `cargo xtask no-fs`.
...
19. **Tool dispatch layer ecosystem is `D`-free** — `Tool<D = ()>` carries an operator-supplied typed `D` (defaults to `()` for the deps-less zero-cost path) that flows through `AgentContext<D>::deps()` to leaf execution. `ToolRegistry<D = ()>` holds a `D` clone that the leaf `InnerToolService<D>` threads into every `Tool<D>::execute` invocation. The layer ecosystem (`PolicyLayer`, `OtelLayer`, `RetryService`, `ApprovalLayer`, `ScopedToolLayer`, `ToolEventLayer`) consumes the `D`-free `ToolInvocation` (carries `ExecutionContext`, no `D`) so existing layer impls compile unchanged regardless of operator deps. The proc-macro `#[tool]` (in `entelix-tool-derive`) generates `SchemaTool` impls from an `async fn(ctx: &AgentContext<D>, ...args) -> Result<O>` so typed-input authoring stays one-line.
