...
22. **Provider IR promotion criterion** — a vendor knob lands on `ModelRequest` (cross-provider IR) when **two or more first-party codecs** carry the concept natively. A knob with one native vendor lives on a per-vendor `*Ext` (e.g. `OpenAiResponsesExt::reasoning_summary`). Codecs that lack the concept emit `ModelWarning::LossyEncode { field, detail }`; codecs whose wire shape inverts the polarity translate at encode (Anthropic / Bedrock-Anthropic invert `parallel_tool_calls` into `tool_choice.disable_parallel_tool_use`). The criterion prevents ad-hoc IR growth and keeps IR field semantics meaningful across the codec matrix; it also ensures third-party codecs only need to handle IR fields, never every vendor's bespoke ext. New IR field PRs must cite the two native vendors in the doc-comment.
