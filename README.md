...
- **Five memory patterns over `Store<V>`** — `BufferMemory`, `SummaryMemory`, `EntityMemory`, `SemanticMemory<E, V>`, `EpisodicMemory<E, V>` (time-ordered episode log). All compose over the `Store<V>` trait — first-class tiers are `Store` / `VectorStore` / `GraphMemory` / `Checkpointer` / `SessionLog`; these five are convenience patterns over Tier 3 (`Store`). Multi-tenant `Namespace` with mandatory non-empty `tenant_id` (compile-time + runtime enforced).
