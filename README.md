...
[![Rust 1.95](https://img.shields.io/badge/rust-1.95-orange)](https://www.rust-lang.org)
...
- **Memory patterns over `Store<V>`** — `BufferMemory`, `SummaryMemory`, `EntityMemory`, `SemanticMemory<E, V>`, `EpisodicMemory<V>` (time-ordered episode log). All compose over the `Store<V>` trait — first-class tiers are `Store` / `VectorStore` / `GraphMemory` / `Checkpointer` / `SessionLog`; the patterns are convenience facades over Tier 3 (`Store`). Multi-tenant `Namespace` with mandatory non-empty `tenant_id` (compile-time + runtime enforced).
...
- **Typed audit channel** — `entelix::AuditSink` with typed `record_*` verbs (`sub_agent_invoked` / `agent_handoff` / `resumed` / `memory_recall` / `usage_limit_exceeded` / `context_compacted`). `entelix::SessionAuditSink` maps onto `GraphEvent` so replays reconstruct managed-agent lifecycle without re-running the dispatch path.
