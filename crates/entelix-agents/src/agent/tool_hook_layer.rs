...
/// Internal layer for pre-dispatch hook decisions.
#[derive(Clone, Default)]
pub(crate) struct ToolBeforeHookLayer {
    hooks: ToolHookRegistry,
}

impl ToolBeforeHookLayer {
    /// Construct a pre-dispatch hook layer.
    #[must_use]
    pub(crate) const fn new(hooks: ToolHookRegistry) -> Self {
        Self { hooks }
    }
}

impl<Inner> Layer<Inner> for ToolBeforeHookLayer {
    type Service = ToolBeforeHookService<Inner>;

    fn layer(&self, inner: Inner) -> Self::Service {
        ToolBeforeHookService {
            inner,
            hooks: self.hooks.clone(),
        }
    }
...
/// Service produced by [`ToolBeforeHookLayer`].
pub(crate) struct ToolBeforeHookService<Inner> {
    inner: Inner,
    hooks: ToolHookRegistry,
}

impl<Inner: Clone> Clone for ToolBeforeHookService<Inner> {
    fn clone(&self) -> Self {
        Self {
            inner: self.inner.clone(),
            hooks: self.hooks.clone(),
...
impl<Inner> Service<ToolInvocation> for ToolBeforeHookService<Inner>
where
    Inner: Service<ToolInvocation, Response = Value, Error = Error> + Clone + Send + 'static,
    Inner::Future: Send + 'static,
{
    ...
    fn call(&mut self, mut invocation: ToolInvocation) -> Self::Future {
        ...
        let inner = self.inner.clone();
        Box::pin(async move {
            hooks.apply_before(&mut invocation).await?;
            inner.oneshot(invocation).await
        })
    }
}

/// Internal layer for post-dispatch hook observation and validation.
#[derive(Clone, Default)]
pub(crate) struct ToolAfterHookLayer {
    hooks: ToolHookRegistry,
}

impl ToolAfterHookLayer {
    /// Construct a post-dispatch hook layer.
    #[must_use]
    pub(crate) const fn new(hooks: ToolHookRegistry) -> Self {
        Self { hooks }
    }
...
impl<Inner> Layer<Inner> for ToolAfterHookLayer {
    type Service = ToolAfterHookService<Inner>;

    fn layer(&self, inner: Inner) -> Self::Service {
        ToolAfterHookService {
            inner,
            hooks: self.hooks.clone(),
        }
    }
...
/// Service produced by [`ToolAfterHookLayer`].
pub(crate) struct ToolAfterHookService<Inner> {
    inner: Inner,
    hooks: ToolHookRegistry,
}

impl<Inner: Clone> Clone for ToolAfterHookService<Inner> {
    fn clone(&self) -> Self {
        Self {
            inner: self.inner.clone(),
            hooks: self.hooks.clone(),
...
impl<Inner> Service<ToolInvocation> for ToolAfterHookService<Inner>
where
    Inner: Service<ToolInvocation, Response = Value, Error = Error> + Clone + Send + 'static,
    Inner::Future: Send + 'static,
{
    ...
    fn call(&mut self, invocation: ToolInvocation) -> Self::Future {
        ...
        let inner = self.inner.clone();
        Box::pin(async move {
            let request = ToolHookRequest::from_invocation(&invocation);
            let ctx = invocation.ctx.clone();
            match inner.oneshot(invocation).await {
                Ok(output) => match hooks.apply_after(&request, &output, &ctx).await {
                    Ok(()) => Ok(output),
                    Err(error) => {
                        hooks.apply_error(&request, &error, &ctx).await;
                        Err(error)
