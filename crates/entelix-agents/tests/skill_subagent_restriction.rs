...
//!
//! A sub-agent must declare which skills it inherits. The explicit
//! allowlist narrows the parent's authority; names outside the
//! allowlist are removed, and names absent from the parent are
//! rejected at construction with `Error::Config` so a typo surfaces
//! at the moment it is introduced rather than as a silent "skill not
//! found" at runtime.
...
#[test]
fn subagent_with_skills_rejects_empty_names() {
    let parent = parent_skills();
    let err = Subagent::builder(
        StubModel,
        &ToolRegistry::new(),
    ...
    .restrict_to(&[])
    .with_skills(&parent, &["echo", " "])
    .build()
    .unwrap_err();
    let msg = format!("{err}");
    assert!(
        msg.contains("must not be empty"),
        "error must identify empty skill name, got: {msg}"
    );
}
