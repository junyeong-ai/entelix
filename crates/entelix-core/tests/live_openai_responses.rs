//! Live-API smoke for [`OpenAiResponsesCodec`] + [`DirectTransport`].
//!
//! `#[ignore]`-gated. Run with:
//!
//! ```text
//! OPENAI_API_KEY=sk-... \
//!     cargo test -p entelix-core --test live_openai_responses -- --ignored
//! ```
//!
//! Optional override: `ENTELIX_LIVE_OPENAI_RESPONSES_MODEL`
//! (default `gpt-4o-mini`). The Responses API is `POST /v1/responses`
//! with `input` / `output` rather than Chat's `messages` /
//! `choices`; this smoke proves the codec drives the alternate
//! shape end-to-end.
...
async fn openai_responses_minimal_round_trip() {
    let api_key =
        std::env::var("OPENAI_API_KEY").expect("set OPENAI_API_KEY to run this live-API smoke");
    let model = std::env::var("ENTELIX_LIVE_OPENAI_RESPONSES_MODEL")
    ...
    let encoded = codec.encode(&request).expect("openai responses encode");
    let response = transport
        .send(encoded, &ExecutionContext::new())
        .await
        .expect("openai responses send");
    ...
    let decoded = codec
        ...
        .expect("openai responses decode");
    assert!(
        decoded
            .content
            .iter()
