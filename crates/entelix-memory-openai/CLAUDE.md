...
- **`Arc<Self>` constraint** — every `Embedder` impl must be cheap to clone (F10 mitigation). `OpenAiEmbedder` holds `Arc<reqwest::Client>` internally so `clone()` doesn't reconstruct the connection pool.
