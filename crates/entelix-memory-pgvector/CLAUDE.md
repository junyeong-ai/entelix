...
- **Row-level security mandatory** — `tenant_id` column + `enable_rls` migration helper + `set_tenant_session(tenant_id)` per-call wrap. Mirrors `entelix-persistence` and `entelix-graphmemory-pg` RLS pattern (companion-family uniformity).
