...
- **`prelude` module** — `use entelix::prelude::*` brings the headline conversation surface (`ContentPart` / `Message` / `Role` / `AgentContext` / `ChatModel` / `Error` / `ExecutionContext` / `Result`), the prompt builders (`ChatPromptPart` / `ChatPromptTemplate` / `PromptValue` / `PromptVars`), the composition trio (`Runnable` / `RunnableExt` / `JsonOutputParser`), plus the RAG primitives (`Document` / `IngestionPipeline` / `TextSplitter` family / `Chunker` / `Retriever` / corrective-RAG types + their `DEFAULT_*` constants) so a RAG agent compiles from one glob import. The non-prelude path stays available for callers who want only a subset — explicit `use entelix::{ChatModel, ...}` always works.
